...
const PRIVACY_COPY = {
  en: {
      ...
      body: (
        <>
          ...
          <p>
            ...
            earlier analyses — at most the last twelve, sent individually. Each
            of those answers can include the reason the AI gave, the post it
            proposed, the version you wrote if you edited it, a reason you gave
            if you gave one, and a short extract of the piece it was about.
          </p>
          <p>
            When answers older than those twelve exist, the request also carries
            a written summary of them rather than the answers themselves. The
            next section describes that summary and how it is made.
          </p>
          <p>
    ...
    creatorMemory: {
      title: "The summary of your older answers",
      body: (
        <>
          <p>
            Only your twelve most recent answers are sent individually. Older
            ones are not simply dropped: Koqentra keeps{" "}
            <strong>
              a single written summary of them, generated by Anthropic
            </strong>
            , and sends that summary with later analyses in place of the
            individual answers it covers.
          </p>
          <p>
            Building and extending that summary is a second, separate request to
            Anthropic. It happens while an analysis you asked for is running,
            and it carries the previous summary when one exists together with
            the older answers of yours that have not been folded into it yet —
            the same details listed above for a recent answer. It never carries
            your Creator preferences, the piece you are analysing now, or your
            twelve most recent answers.
          </p>
          <p>
            Older answers are incorporated a batch at a time, so if you have a
            long history the summary catches up over several analyses rather
            than all at once.
          </p>
          <p>
            Koqentra stores the summary, the number of older answers it was
            built from, and{" "}
            <strong>
              a record of which of your older answers have already been
              incorporated into it
            </strong>
            , so that the same answer is not incorporated twice. That record
            identifies which of your answers have been used; it holds no copy of
            what you wrote.
          </p>
          <p>
            <strong>The summary is written by an AI, not by you.</strong> It is
            an inference drawn from what you did, it can be wrong or incomplete,
            and it is not a statement you made. Your Creator preferences and
            your recent answers take priority over it. There is no way to edit,
            rebuild or delete it from inside Koqentra, and the retention limits
            described below apply to it as they do to everything else here.
          </p>
        </>
      ),
    },
  ...
  ja: {
    ...
    creator: {
      ...
      body: (
        <>
          ...
          <p>
            このリクエストには、Koqentraに登録したオーディエンス・目標・文体の指示に加えて、過去の分析に対する直近の回答も一緒に送られます。個別に送られるのは
            <strong>最大で直近12件</strong>
            です。それぞれの回答には、AIが示した理由、AIが提案した投稿文、利用者が編集した場合はその文章、利用者が理由を入力した場合はその理由、および対象だったコンテンツの短い抜粋が含まれることがあります。
          </p>
          <p>
            その12件より古い回答がある場合、リクエストにはそれらの回答そのものではなく、まとめた要約が含まれます。要約の内容と作られ方は次の節で説明します。
          </p>
          <p>
    ...
    creatorMemory: {
      ...
      body: (
        <>
          <p>
            個別に送られるのは直近12件の回答だけですが、それより古い回答が単に
            捨てられるわけではありません。Koqentraは
            <strong>それらをまとめた要約をAnthropicに生成させて保持し</strong>
            、以降の分析では、要約が代表している個別の回答の代わりにその要約を
            送信します。
          </p>
          <p>
            この要約の作成と更新は、Anthropicへの別のリクエストです。利用者が
            依頼した分析の実行中に行われ、既存の要約がある場合はそれと、まだ
            要約へ取り込まれていない古い回答（内容は上記の「直近の回答」と同じ
            項目）が送信されます。クリエイター設定、今回分析するコンテンツ、直近12件の
            回答が、このリクエストに含まれることはありません。
          </p>
          <p>
            古い回答は一度にまとめてではなく、分割して取り込まれます。履歴が
            長い場合、要約は複数回の分析にわたって少しずつ追いつきます。
          </p>
          <p>
            Koqentraは、要約そのものと、それが何件の古い回答から作られたかに
            加えて、
            <strong>どの古い回答をすでに要約へ取り込んだかの記録</strong>
            を保存します。同じ回答を重複して要約へ取り込まないための記録で、
            どの回答を取り込んだかを示すだけであり、利用者が書いた内容は
            含まれません。
          </p>
          <p>
            <strong>この要約は利用者本人ではなくAIが書いたものです。</strong>
            利用者の行動から導かれた推論であり、誤っていたり不完全であったり
            することがあります。利用者が明示した記述ではありません。
            クリエイター設定と直近の回答のほうが優先されます。Koqentraの中から
            この要約を編集・再作成・削除する手段はなく、以下に記載する保持に
            関する制約は、この要約にも同じように当てはまります。
          </p>
        </>
      ),
    },
  ...
  return (
    <div className="flex flex-1 flex-col bg-background">
      ...
      <main className="mx-auto w-full max-w-2xl flex-1 px-6 py-10 sm:px-10">
        ...
        {/* **A second thing sent to Anthropic, and a second thing stored.**
            Older answers stopped simply falling out of the window in C2.0B;
            they are summarised instead, by a separate request, and the summary
            is kept. Both halves are disclosed because both are new. */}
        <Section title={copy.creatorMemory.title}>
          {copy.creatorMemory.body}
        </Section>
