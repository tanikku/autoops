...
A worker's schedule is two values. **`frequency`** says how often — `manual`,
`daily`, `weekly`, `monthly` — and **`runAtMinutes`** says at what time, as
minutes into the day: `0` is midnight, `540` is 09:00.

The time is read in the **owner's timezone**, so 09:00 means 09:00 wherever the
account is set to (see [Settings](#settings)). What is stored in `nextRunAt` is
the UTC instant that lands on.

`runAtMinutes` is optional. Without it the worker keeps whatever time its slot
already had — the behaviour every worker had before a time could be chosen, and
what rows created then still get. A `manual` worker has no slot to place a time
in, so setting one is discarded.

The label on the card — "Every day at 09:00", "Manual execution" — is generated
by `lib/schedule-label.ts` from those two values rather than stored. There used
to be a free-text `schedule` column beside them, typed by the user and
reconciled with nothing: a worker running daily could describe itself as weekly
and the app would show the lie on the card. It was dropped once the fields
carrying a real time arrived to replace it.
...
**Intervals with a time are counted in calendar days, not in hours.** Adding 24
hours would hold the stored instant still while the wall clock moved, and
"every day at 09:00" would quietly become 10:00 for half the year in any zone
observing daylight saving. Stepping the local date and converting back to UTC
keeps the hour the user chose.
...
What frequency still cannot express is **which day**: a weekly worker keeps the
weekday it was saved on, and a monthly one keeps the date.
...
```
Scheduler  ──►  Dispatcher  ──►  Schedule  ──►  nextRunAt
 what is due     hand-off &        when the       the pending
                 advance           next slot      slot, in UTC
                                   falls
```
...
| **Schedule** (`lib/schedule.ts`) | Computes *when* the next slot falls, from the frequency, the time of day and the owner's zone. |
...
It reads neither `frequency` nor `runAtMinutes` nor a timezone. Everything a
schedule means is resolved into `nextRunAt` before it gets there, which is why
adding a time of day left this file untouched. Keep it that way: a condition
added here is a decision moved out of the module that owns it.
...
- Manual Run
- Scheduled Run — `daily`, `weekly` or `monthly`, at a chosen time of day
- Scheduler — decides what is due
...
| **Routine** | A worker | `frequency` and `runAtMinutes` define the schedule; `nextRunAt` is the pending slot |
...
| Column | Type | Meaning |
| --- | --- | --- |
| `User.timezone` | `String` (default `"UTC"`) | IANA zone. Decides how timestamps read *and* what a chosen time of day means |
| `Routine.frequency` | `String` (default `"manual"`) | How often |
| `Routine.runAtMinutes` | `Int?` | Minutes into the day, in the owner's zone. Null keeps the slot's existing time |
| `Routine.nextRunAt` | `DateTime?` | The pending slot, in UTC. Null means never due |
...
| Sprint 12 | Documentation sync | In Progress |
...
- **A day of the week.** A weekly worker picks its time but not its day: it
  keeps whichever weekday it was saved on. A `runAtWeekday` column alongside
  `runAtMinutes` closes it in the same shape — the calculation already steps
  through local dates, so it is a matter of choosing which one to land on.
- **A day of the month.** The same for monthly workers, which keep the date
  they were saved on. `runAtDay`, with the month-end question below to settle
  as part of it.
